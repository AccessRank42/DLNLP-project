...
# from https://github.com/abisee/cs224n-win18-squad/blob/master/code/data_batcher.py
...
from preprocessing.vocab import PAD_ID, UNK_ID
...
def refill_batches(batches, word2id, context_file, qn_file, span_file, batch_size, context_len, question_len, discard_long):
    """
    ...
    Inputs:
      ...
      context_file, qn_file, span_file: paths to {train/dev}.{context/question/span} data files
    ...
    context_line, qn_line, ans_line = context_file.readline(), qn_file.readline(), span_file.readline() # read the next line from each
    ...
    while context_line and qn_line and ans_line: # while you haven't reached the end
        ...
        context_line, qn_line, ans_line = context_file.readline(), qn_file.readline(), span_file.readline()

        # get ans_tokens from ans_span
        assert len(ans_span) == 2
        if ans_span[1] < ans_span[0]:
            print("Found an ill-formed gold span: start={} end={}".format(ans_span[0], ans_span[1]))
            continue
    ...
    toc = time.time()
    print("Refilling batches took {} seconds".format(toc-tic))
    return
